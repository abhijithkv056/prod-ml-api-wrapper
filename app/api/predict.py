...
async def predict(req: PredictRequest) -> PredictResponse:
    probability = predict_fraud_probability(req.transaction.transaction_amount)
...
async def predict_batch(req: BatchPredictRequest) -> BatchPredictResponse:
    predictions = [
        PredictResponse(fraud_probability=predict_fraud_probability(t.transaction_amount))
